...
export async function placeOrderAction(formData) {
  try {
    ...
    // Supabase schema may have bigint PKs without identity; generate IDs safely for this project.
    const { data: maxOrderRow } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    const nextOrderId = Number(maxOrderRow?.order_id || 0) + 1;
    ...
    const { data: maxItemRow } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    let nextOrderItemId = Number(maxItemRow?.order_item_id || 0) + 1;
    ...
    const msg = `Could not place order: ${error.message || "unknown error"}`;
    redirect(`/place-order?status=error&message=${encodeURIComponent(msg)}`);
  }
}
