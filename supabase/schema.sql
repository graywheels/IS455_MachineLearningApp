-- IS455 ML App — Supabase schema
-- Run this in the Supabase SQL Editor to create all tables from scratch.
-- If you already have an existing order_predictions table with the OLD
-- late-delivery column names, run the migration block below instead.
...
-- Prediction contract: fraud detection (aligned with model_training.ipynb)
...
create index if not exists idx_predictions_fraud_prob
  on order_predictions(fraud_probability desc);

-- ── Migration (existing deployments only) ────────────────────────────────────
-- If your order_predictions table already exists with the old late-delivery
-- column names, run these two statements instead of recreating the table:
--
...
--
-- Then re-run the index creation above.
-- ─────────────────────────────────────────────────────────────────────────────
